...
import { Item, ItemResponse, ItemView } from 'src/app/shared/interfaces/item.interface';
...
describe('ItemsComponent', () => {
  ...
  it('should get items, categories and brands on init', () => {
    ...
    const mockItemView: ItemView[] = [
      {
        id: mockItemResponse.content[0].id,
        name: mockItemResponse.content[0].name,
        description: mockItemResponse.content[0].description,
        price: mockItemResponse.content[0].price,
        stock: mockItemResponse.content[0].stock,
        brand: mockItemResponse.content[0].brand.name,
        categories: mockItemResponse.content[0].categories.map(category => category.name).join(', ')
      }
    ]
    ...
    expect(component.items).toEqual(mockItemView);
  ...
  describe('AddSupply', () => {
    ...
    it('should add supply successfully', () => {
      ...
      component.items = [
        {
          ...
          categories: currentItem.categories.map(category => category.name).join(', ')
        }
      ]
      ...
      expect(component.items[0].stock).toBe(currentItem.stock);
  ...
  it('should change sort param', () => {
    ...
    component.changeSortParam('brand');

    expect(component.sortParam).toBe('brand');
  });
