...
@Component({
  ...
  styleUrls: ['./register-user-client.component.scss']
})
export class RegisterUserClientComponent implements OnInit {
  ...
  get name(): AbstractControl | null {
    return this.registerForm.get(USER_FIELD_NAMES.USER_NAME[0]); 
 }

 get lastName(): AbstractControl | null {
   return this.registerForm.get(USER_FIELD_NAMES.USER_LAST_NAME[0]);
 }

 get identityDocument(): AbstractControl | null {
   return this.registerForm.get(USER_FIELD_NAMES.USER_IDENTITY_DOCUMENT[0]);
 }

 get phone(): AbstractControl | null {
   return this.registerForm.get(USER_FIELD_NAMES.USER_PHONE[0]);
 }

 get email(): AbstractControl | null {
   return this.registerForm.get(USER_FIELD_NAMES.USER_EMAIL[0]);
 }

 get password(): AbstractControl | null {
   return this.registerForm.get(USER_FIELD_NAMES.USER_PASSWORD[0]);
 }

 get birthDate(): AbstractControl | null {
   return this.registerForm.get(USER_FIELD_NAMES.USER_BIRTH_DATE[0]);
 }
 ...
 onSubmit(): void {
  ...
  const assistant: User = this.registerForm.value;
  assistant.birthDate = formatDate(assistant.birthDate);

  this.userService
    .createUserClient(assistant)
    .subscribe({
      next: (response) => {
